...
class BiGCNet(nn.Module):

    def __init__(self, n_items, n_buckets, emb_size, n_topics,
                 n_heads, n_branches, n_layers, n_samples_per_layer=16,
                 *, dropout=0.5, slope=0.2, temporal_concat=False, score_layer_hidden=False,
                 dot_attn=True, use_mid_layer=True, bias=True):
        super().__init__()
        ...
        if temporal_concat:
            self.in_proj_layer = nn.Sequential(nn.Dropout(dropout),
        ...
        #         self.mid_layers = [nn.Sequential(nn.Dropout(dropout),
        #                                          nn.Linear(n_heads * n_branches * emb_size, emb_size),
        #                                          nn.LeakyReLU(slope))
        #                            for _ in range(n_layers)]
        self.mid_layers = [lambda x: x for _ in range(n_layers)]
        self.bigc_layers = nn.ModuleList(self.bigc_layers)
        #         self.mid_layers = nn.ModuleList(self.mid_layers)
        self.layer_norm_layers = nn.ModuleList([nn.LayerNorm(emb_size) for _ in range(n_layers)])
        if score_layer_hidden:
            ...
            self.score_layer = nn.Sequential(nn.Dropout(dropout),
                                             nn.Linear(emb_size * (1 + n_layers), 1, bias=False))

    def in_proj(self, item_emb, temp_emb):
        if self.temporal_concat:
            return self.in_proj_layer(torch.cat([item_emb, temp_emb], dim=-1))
        else:
    ...
    def forward(self, h_iids, buckets_ids):
        ...
        x = self.in_proj(item_emb, temp_emb)
        z = self.item_emb_table.weight.expand(bs, -1, -1)

        outputs = [z]
        for i, (bigc_f, affine_f, layer_norm) in enumerate(
                zip(self.bigc_layers, self.mid_layers, self.layer_norm_layers)):
            mask = None if i else h_iids.eq(0)
            #             z1 = layer_norm(affine_f(bigc_f(x, z, mask)) + z)
            z1 = bigc_f(x, z, mask)
            outputs.append(z1)
            #             x = layer_norm(z + z1)
            x = z1
        ...
        logits = self.score_layer(outputs).squeeze(-1)
        return logits
    ...
    def sample_forward_full_return(self, h_iids, buckets_ids):
        ...
        item_emb = self.item_emb_table(h_iids)
        temp_emb = self.temporal_emb_table(buckets_ids)
        x = self.in_proj(item_emb, temp_emb)  # [bs, len, dim]
        ...
        for i, (bigc_f, affine_f, layer_norm) in enumerate(
            ...
            #             x = layer_norm(z1[:, idx] + z[:, idx])  # [bs, n_samples_per_layer, dim]
            x = z1[:, idx]
            z = z1
    ...
    def mini_batch_forward(self, h_iids, buckets_ids, t_iids):
        ...
        for i, (bigc_f, affine_f, layer_norm) in enumerate(
            ...
            #             x = layer_norm(x + z[:, -n_samples_per_layer:])
