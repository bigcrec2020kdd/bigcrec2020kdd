...
class BiGCrec_v4(nn.Module):
    ...
    def __init__(self, n_items, n_buckets, emb_size, n_topics, n_heads, n_branches, n_layers,
        ...
        self.attn_layer = nn.Sequential(nn.Linear(emb_size, 1, bias=False),
                                        nn.Softmax(dim=1))
        self.base_weight = nn.Parameter(torch.Tensor(1, emb_size))
        nn.init.zeros_(self.base_weight.data)
        self.gate_logit = nn.Parameter(torch.Tensor(1))
        nn.init.zeros_(self.gate_logit.data)
    ...
    def get_score(self, outputs, seq_emb):
        # seq_emb : [bs, len, dim]
        p_attn = self.attn_layer(seq_emb)  # [bs, len, 1]
        context = (seq_emb * p_attn).sum(dim=1)  # [bs, dim]
        gate = torch.sigmoid(self.gate_logit)
        context = gate * context + (1. - gate) * self.base_weight
